...
use crate::zkevm_circuits::bn254::ec_add::input::EcAddCircuitFSMInputOutputWitness;
use crate::zkevm_circuits::bn254::ec_mul::input::{EcMulCircuitFSMInputOutputWitness, EcMulCircuitInputOutputWitness, EcMulCircuitInstanceWitness};
use crate::zkevm_circuits::bn254::ec_pairing::input::{EcPairingCircuitFSMInputOutputWitness, EcPairingCircuitInputOutputWitness, EcPairingCircuitInstanceWitness, EcPairingFunctionFSMWitness};
...
pub fn ecpairing_decompose_into_per_circuit_witness<
    F: SmallField,
    R: BuildableCircuitRoundFunction<F, 8, 12, 4> + AlgebraicRoundFunction<F, 8, 12, 4>,
>(
    artifacts: &mut FullBlockArtifacts<F>,
    mut demuxed_ecpairing_queue: LogQueue<F>,
    ...
    // let mut ecpairing_memory_queries = vec![];
    //
    // for (_cycle, _query, witness) in artifacts.ecpairing_witnesses.iter() {
    ...
    // let mut result = vec![];
    //
    // let precompile_calls = std::mem::replace(&mut artifacts.demuxed_ecpairing_queries, vec![]);
    // let precompile_calls_queue_states =
    //     std::mem::replace(&mut demuxed_ecpairing_queue.states, vec![]);
    // let simulator_witness: Vec<_> = demuxed_ecpairing_queue.simulator.witness.clone().into();
    // let round_function_witness = std::mem::replace(&mut artifacts.ecpairing_witnesses, vec![]);
    //
    // let memory_queries = ecpairing_memory_queries;
    //
    ...
    //                     internal_fsm: EcPairingFunctionFSMWitness {
    //                         read_precompile_call: false,
    //                         read_words_for_round: false,
    //                         completed: false,
    //                         pairing_inner_state: ((((), ()), ((), ()), ((), ())), (((), ()), ((), ()), ((), ()))),
    //                         timestamp_to_use_for_read: 0,
    //                         timestamp_to_use_for_write: 0,
