...
use crate::zkevm_circuits::bn254::ec_add::input::{EcAddCircuitFSMInputOutputWitness, EcAddCircuitInputOutputWitness, EcAddCircuitInstanceWitness};
...
pub fn ecadd_decompose_into_per_circuit_witness<
    F: SmallField,
    R: BuildableCircuitRoundFunction<F, 8, 12, 4> + AlgebraicRoundFunction<F, 8, 12, 4>,
>(
    artifacts: &mut FullBlockArtifacts<F>,
    mut demuxed_ecadd_queue: LogQueue<F>,
    ...
    let mut ecadd_memory_queries = vec![];

    for (_cycle, _query, witness) in artifacts.ecadd_witnesses.iter() {
        let ECAddRoundWitness {
            new_request: _,
            reads,
            writes,
        } = witness;
        ...
        // we read, then write
        ecadd_memory_queries.extend_from_slice(reads);

        ecadd_memory_queries.extend_from_slice(writes);
    }

    let mut result = vec![];

    let precompile_calls = std::mem::replace(&mut artifacts.demuxed_ecadd_queries, vec![]);
    let precompile_calls_queue_states =
        std::mem::replace(&mut demuxed_ecadd_queue.states, vec![]);
    let simulator_witness: Vec<_> = demuxed_ecadd_queue.simulator.witness.clone().into();
    let round_function_witness = std::mem::replace(&mut artifacts.ecadd_witnesses, vec![]);

    let memory_queries = ecadd_memory_queries;
    ...
    let mut log_queue_input_state =
        take_queue_state_from_simulator(&demuxed_ecadd_queue.simulator);
