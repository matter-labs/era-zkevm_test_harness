...
>(
...
) -> (PubdataHasherInstanceWitness<E, 3, 64, InitialStorageWriteData<E>>, PubdataHasherInstanceWitness<E, 2, 40, RepeatedStorageWriteData<E>>) {
    let actual_keys: Vec<_> = artifacts.deduplicated_rollup_storage_queries.iter()
        .filter(|el| el.rw_flag)
        .map(|el| el.derive_final_address()).collect();

    let leafs = artifacts.deduplicated_rollup_storage_queries.iter()
        .filter(|el| el.rw_flag)
        .map(|el| {
    ...
    let first_writes_circuit_witness = compute_pubdata_hasher_witness(
        &first_writes_simulator,
        first_writes_capacity
    );

    let repeated_writes_circuit_witness = compute_pubdata_hasher_witness(
        &repeated_writes_simulator,
        repeated_writes_capacity
    );

    (first_writes_circuit_witness, repeated_writes_circuit_witness)
}
