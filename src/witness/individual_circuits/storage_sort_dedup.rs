...
>(
    ...
    let rhs_contributions: Vec<_> = intermediate_sorted_log_simulator.witness.iter().map(|el| el.0).collect();
    ...
    assert!(artifacts.demuxed_rollup_storage_queue_simulator.witness.as_slices().1.is_empty());
    assert!(intermediate_sorted_log_simulator.witness.as_slices().1.is_empty());
