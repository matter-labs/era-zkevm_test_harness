...
>(
    ...
    let mut demuxed_keccak_precompile_queries_it = artifacts.demuxed_keccak_precompile_queries.iter();
    let mut demuxed_sha256_precompile_queries_it = artifacts.demuxed_sha256_precompile_queries.iter();
    ...
    for (idx, input_chunk) in input_queue_witness.chunks(per_circuit_capacity).enumerate() {
            ...
            let (_, states) = states_iter.next().unwrap();
                match query.aux_byte {
                    STORAGE_AUX_BYTE => {
                        ...
                        match query.shard_id {
                            0 => {
                                ...
                                let (_old_tail, intermediate_info) = artifacts.demuxed_rollup_storage_queue_simulator.push_and_output_intermediate_data(
                                    item,
                                    round_function
                                );
                    
                                artifacts.demuxed_rollup_storage_queue_states.push(intermediate_info);
                            },
                            _ => unreachable!(),
                        }
                    },
                    L1_MESSAGE_AUX_BYTE => {
                        ...
                        let (_old_tail, intermediate_info) = artifacts.demuxed_to_l1_queue_simulator.push_and_output_intermediate_data(
                            item,
                            round_function
                        );
                    ...
                    EVENT_AUX_BYTE => {
                        ...
                        let (_old_tail, intermediate_info) = artifacts.demuxed_events_queue_simulator.push_and_output_intermediate_data(
                            item,
                            round_function
                        );
                    ...
                    PRECOMPILE_AUX_BYTE => {
                        ...
                        match query.address {
                            a if a == *KECCAK256_ROUND_FUNCTION_PRECOMPILE_FORMAL_ADDRESS => {
                                ...
                                let (_old_tail, intermediate_info) = artifacts.demuxed_keccak_precompile_queue_simulator.push_and_output_intermediate_data(
                                    item,
                                    round_function
                                );
                    
                                artifacts.demuxed_keccak_precompile_queue_states.push(intermediate_info);
                            }
                            a if a == *SHA256_ROUND_FUNCTION_PRECOMPILE_FORMAL_ADDRESS => {
                                let item = demuxed_sha256_precompile_queries_it.next().copied().unwrap();
                                let (_old_tail, intermediate_info) = artifacts.demuxed_sha256_precompile_queue_simulator.push_and_output_intermediate_data(
                                    item,
                                    round_function
                                );
                    
                                artifacts.demuxed_sha256_precompile_queue_states.push(intermediate_info);
                            }
                            a if a == *ECRECOVER_INNER_FUNCTION_PRECOMPILE_FORMAL_ADDRESS => {
                                let item = demuxed_ecrecover_queries_it.next().copied().unwrap();
                                let (_old_tail, intermediate_info) = artifacts.demuxed_ecrecover_queue_simulator.push_and_output_intermediate_data(
                                    item,
                                    round_function
                                );
                    
                                artifacts.demuxed_ecrecover_queue_states.push(intermediate_info);
                            }
                            _ => {
                                // just burn ergs
                            },
        ...
        fsm_output.events_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_events_queue_simulator);
        fsm_output.l1messages_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_to_l1_queue_simulator);
        fsm_output.keccak256_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_keccak_precompile_queue_simulator);
        fsm_output.sha256_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_sha256_precompile_queue_simulator);
        fsm_output.ecrecover_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_ecrecover_queue_simulator);
        ...
        if is_last {
            witness.closed_form_input.observable_output.storage_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_rollup_storage_queue_simulator);
            witness.closed_form_input.observable_output.events_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_events_queue_simulator);
            witness.closed_form_input.observable_output.l1messages_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_to_l1_queue_simulator);
            witness.closed_form_input.observable_output.keccak256_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_keccak_precompile_queue_simulator);
            witness.closed_form_input.observable_output.sha256_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_sha256_precompile_queue_simulator);
            witness.closed_form_input.observable_output.ecrecover_access_queue_state = take_queue_state_from_simulator(&artifacts.demuxed_ecrecover_queue_simulator);
        }

        if is_last {
            assert_eq!(
                &witness.closed_form_input.observable_output.storage_access_queue_state, 
                &witness.closed_form_input.hidden_fsm_output.storage_access_queue_state, 
            );
        }

        if let Some(previous_witness) = results.last() {
            witness.closed_form_input.hidden_fsm_input = previous_witness.closed_form_input.hidden_fsm_output.clone();
        }
