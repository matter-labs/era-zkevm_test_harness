...
use circuit_definitions::encodings::*;use crate::zkevm_circuits::bn254::ec_mul::input::{EcMulCircuitFSMInputOutputWitness, EcMulCircuitInputOutputWitness, EcMulCircuitInstanceWitness};
...
pub fn ecmul_decompose_into_per_circuit_witness<
    F: SmallField,
    R: BuildableCircuitRoundFunction<F, 8, 12, 4> + AlgebraicRoundFunction<F, 8, 12, 4>,
>(
    artifacts: &mut FullBlockArtifacts<F>,
    mut demuxed_ecmul_queue: LogQueue<F>,
    num_rounds_per_circuit: usize,
    round_function: &R,
) -> Vec<EcMulCircuitInstanceWitness<F>> {
    assert_eq!(
    ...
    let mut ecmul_memory_queries = vec![];

    for (_cycle, _query, witness) in artifacts.ecmul_witnesses.iter() {
        let ECMulRoundWitness {
            new_request: _,
            reads,
            writes,
        } = witness;
        ...
        // we read, then write
        ecmul_memory_queries.extend_from_slice(reads);

        ecmul_memory_queries.extend_from_slice(writes);
    }

    let mut result = vec![];

    let precompile_calls = std::mem::replace(&mut artifacts.demuxed_ecmul_queries, vec![]);
    let precompile_calls_queue_states =
        std::mem::replace(&mut demuxed_ecmul_queue.states, vec![]);
    let simulator_witness: Vec<_> = demuxed_ecmul_queue.simulator.witness.clone().into();
    let round_function_witness = std::mem::replace(&mut artifacts.ecmul_witnesses, vec![]);

    let memory_queries = ecmul_memory_queries;
    ...
    let mut log_queue_input_state =
        take_queue_state_from_simulator(&demuxed_ecmul_queue.simulator);
        ...
        .enumerate()
    {
        ...
        if round_counter == num_rounds_per_circuit || is_last_request {
            ...
            let witness = EcMulCircuitInstanceWitness::<F> {
                closed_form_input: EcMulCircuitInputOutputWitness::<F> {
                    ...
                    hidden_fsm_input: EcMulCircuitFSMInputOutputWitness::<F> {
                        log_queue_state: log_queue_input_state.clone(),
                        memory_queue_state: memory_queue_input_state,
                    },
                    hidden_fsm_output: EcMulCircuitFSMInputOutputWitness::<F> {
                        log_queue_state: take_queue_state_from_simulator(
                            &demuxed_ecmul_queue.simulator,
                        ),
