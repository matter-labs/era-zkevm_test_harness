...
use crate::zkevm_circuits::modexp::input::{ModexpCircuitInputOutputWitness, ModexpCircuitInstanceWitness};
...
pub fn modexp_decompose_into_per_circuit_witness<
    F: SmallField,
    R: BuildableCircuitRoundFunction<F, 8, 12, 4> + AlgebraicRoundFunction<F, 8, 12, 4>,
>(
    artifacts: &mut FullBlockArtifacts<F>,
    mut demuxed_modexp_queue: LogQueue<F>,
    ...
    let mut modexp_memory_queries = vec![];

    for (_cycle, _query, witness) in artifacts.modexp_witnesses.iter() {
        let ModexpRoundWitness{
            new_request: _,
            reads,
            writes,
        } = witness;
        ...
        // we read, then write
        modexp_memory_queries.extend_from_slice(reads);

        modexp_memory_queries.extend_from_slice(writes);
    }

    let mut result = vec![];

    let precompile_calls = std::mem::replace(&mut artifacts.demuxed_modexp_queries, vec![]);
    let precompile_calls_queue_states =
        std::mem::replace(&mut demuxed_modexp_queue.states, vec![]);
    let simulator_witness: Vec<_> = demuxed_modexp_queue.simulator.witness.clone().into();
    let round_function_witness = std::mem::replace(&mut artifacts.modexp_witnesses, vec![]);

    let memory_queries = modexp_memory_queries;
