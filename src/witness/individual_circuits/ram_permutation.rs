...
>(
    ...
    let challenges = produce_fs_challenges::<F, R, FULL_SPONGE_QUEUE_STATE_WIDTH, {MEMORY_QUERY_PACKED_WIDTH + 1}, 2>(
    ...
    for idx in 0..DEFAULT_NUM_PERMUTATION_ARGUMENT_REPETITIONS {
        let (lhs_grand_product_chain, rhs_grand_product_chain) =
            compute_grand_product_chains::<F, MEMORY_QUERY_PACKED_WIDTH, {MEMORY_QUERY_PACKED_WIDTH + 1}>(&lhs_contributions, &rhs_contributions, &challenges[idx]);

        assert_eq!(lhs_grand_product_chain.len(), artifacts.all_memory_queries_accumulated.len());
        assert_eq!(rhs_grand_product_chain.len(), artifacts.sorted_memory_queries_accumulated.len());
        assert_eq!(lhs_grand_product_chain.len(), artifacts.memory_queue_simulator.witness.len());
        assert_eq!(rhs_grand_product_chain.len(), sorted_memory_queries_simulator.witness.len());
    ...
    for (idx, (((((unsorted_sponge_states, sorted_sponge_states), lhs_grand_product), rhs_grand_product), unsorted_states), sorted_states)) in it.enumerate() {
        ...
        let unsorted_witness = FullStateCircuitQueueRawWitness::<F, zkevm_circuits::base_structures::memory_query::MemoryQuery<F>, FULL_SPONGE_QUEUE_STATE_WIDTH, MEMORY_QUERY_PACKED_WIDTH> {
            elements: unsorted_states.iter().map(|el| {
                let witness = el.2.reflect();
                (witness, el.1)
            }).collect()
        };

        let sorted_witness = FullStateCircuitQueueRawWitness::<F, zkevm_circuits::base_structures::memory_query::MemoryQuery<F>, FULL_SPONGE_QUEUE_STATE_WIDTH, MEMORY_QUERY_PACKED_WIDTH> {
            elements: sorted_states.iter().map(|el| {
                let witness = el.2.reflect();
                (witness, el.1)
            }).collect()
        };
        ...
        assert_eq!(current_unsorted_queue_state.tail.length, current_sorted_queue_state.tail.length);
        ...
        final_unsorted_state.tail.length = unsorted_global_final_state.num_items - final_unsorted_state.tail.length;
        ...
        final_sorted_state.tail.length = sorted_global_final_state.num_items - final_sorted_state.tail.length;

        assert_eq!(final_unsorted_state.tail.length, final_sorted_state.tail.length);
        ...
        if sorted_states.len() % per_circuit_capacity != 0 {
            ...
            instance_witness.closed_form_input.hidden_fsm_output.previous_sorting_key = [0u32; RAM_SORTING_KEY_LENGTH];
            instance_witness.closed_form_input.hidden_fsm_output.previous_full_key = [0u32; RAM_FULL_KEY_LENGTH];
            instance_witness.closed_form_input.hidden_fsm_output.previous_value = U256::zero();
