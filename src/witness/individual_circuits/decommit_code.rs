...
>(
    ...
    let input_witness: VecDeque<_> = unsorted_decommittment_queue_simulator
        .witness
        .iter()
    ...
    let sorted_witness: VecDeque<_> = sorted_decommittment_queue_simulator
        .witness
        .iter()
        .map(|(encoding, old_tail, element)| {
            let wit = DecommitQueryWitness {
    ...
    let decommittments_deduplicator_witness = CodeDecommittmentsDeduplicatorInstanceWitness {
        ...
        intermediate_sorted_queue_state: take_sponge_like_queue_state_from_simulator(
