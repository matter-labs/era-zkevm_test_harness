...
fn main() {
    ...
    println!("Starting performance test for circuit {}", opt.numeric_circuit);
    let start_time = Instant::now();
    get_circuit_capacity(opt.numeric_circuit);
    println!("Finished for circuit {}, took: {} seconds", opt.numeric_circuit, start_time.elapsed().as_secs());
    assert_eq!(true, start_time.elapsed().as_secs() <= get_circuit_to_synthesis_upper_bound_in_seconds(opt.numeric_circuit));
}
