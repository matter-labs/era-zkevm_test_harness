// This bootloader should be used for the 1 compiler test case
// MEMORY LAYOUT
// The inputs number is written in the first word, then from the second word(32 byte) inputs itself are written.
//
// Input structure
// First word contains input type, currently contract call(`0`) and contract deployment(`1`) are supported
// 0-32: the input type
// 0-32: `to` address, lower 20 bytes.
// 32-64: `from` address, lower 20 bytes.
// 64-96: `value` for the call, lower 16 bytes.
// 96-128: calldata length.
// 128-`128+calldata length`: calldata
// `128+calldata length`-`160+calldata length`: expected return data length
// `160+calldata length`-`160+calldata length+expected return data length`: expected return data
