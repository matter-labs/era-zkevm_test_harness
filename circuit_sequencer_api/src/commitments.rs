use circuit_encodings::{
    ...
    LogQueueSimulator, ZkSyncDefaultRoundFunction,
};

use crate::{
    queue::{finalize_queue_state, finalized_queue_state_as_bytes},
...
>(
    ...
    use circuit_encodings::zk_evm::abstractions::*;
    use circuit_encodings::zk_evm::aux_structures::*;
    ...
    for (idx, el) in heap_writes.into_iter().enumerate() {
        let query = MemoryQuery {
            ...
            location: MemoryLocation {
                ...
                page: MemoryPage(
                    circuit_encodings::zk_evm::zkevm_opcode_defs::BOOTLOADER_HEAP_PAGE,
                ),
