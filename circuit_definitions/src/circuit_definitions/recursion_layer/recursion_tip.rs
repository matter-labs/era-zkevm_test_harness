...
use zkevm_circuits::recursion::recursion_tip::*;
...
#[serde(bound = "")]
pub struct RecursionTipCircuit<POW: RecursivePoWRunner<F>> {
    pub witness: RecursionTipInstanceWitness<F, RH, EXT>,
    pub config: RecursionTipConfig<F, H, EXT>,
...
impl<POW: RecursivePoWRunner<F>> RecursionTipCircuit<POW>
where
    [(); <LogQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
    [(); <MemoryQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
    [(); <DecommitQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
    ...
    [(); <RecursionQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
{
    pub fn description() -> String {
        "Recursion tip circuit".to_string()
    }
    ...
    pub fn synthesize_into_cs<CS: ConstraintSystem<F> + 'static>(
        ...
        let input_commitments = recursion_tip_entry_point::<F, CS, R, RH, EXT, TR, CTR, POW>(
            cs,
            witness,
            round_function,
            config,
...
pub type ZkSyncRecursionTipCircuit = RecursionTipCircuit<
    // GoldilocksField,
    // GoldilocksExt2,
...
pub type RecursionTipCircuitBuilder<POW> = CircuitBuilderProxy<F, RecursionTipCircuit<POW>>;
pub type ConcreteRecursionTipCircuitBuilder = RecursionTipCircuitBuilder<NoPow>;
