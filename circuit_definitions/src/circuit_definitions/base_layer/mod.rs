...
#[serde(bound = "")]
pub enum ZkSyncBaseLayerStorage<T: Clone + std::fmt::Debug + serde::Serialize + serde::de::DeserializeOwned> {
    MainVM(T),
...
impl<T: Clone + std::fmt::Debug + serde::Serialize + serde::de::DeserializeOwned> ZkSyncBaseLayerStorage<T> {
    ...
    pub fn from_inner(numeric_type: u8, inner: T) -> Self {
        ...
        match numeric_type {
            ...
            a if a == BaseLayerCircuitType::DecommitmentsFilter as u8 => Self::CodeDecommittmentsSorter(inner),
            ...
            a if a == BaseLayerCircuitType::KeccakPrecompile as u8 => Self::KeccakRoundFunction(inner),
            a if a == BaseLayerCircuitType::Sha256Precompile as u8 => Self::Sha256RoundFunction(inner),
...
pub type ZkSyncBaseLayerCircuitInput<F> = ZkSyncBaseLayerStorage<[F; INPUT_OUTPUT_COMMITMENT_LENGTH]>;
